...
architecture rtl of datapath is
  ...
  signal xr_raw_s : signed(data_width-1 downto 0);
	signal xe_raw_s : signed(data_width-1 downto 0);

	signal xr_dyn_s : signed(data_width-1 downto 0);
	signal xe_dyn_s : signed(data_width-1 downto 0);

	signal mean_xr_s : signed(data_width-1 downto 0);
	signal mean_xe_s : signed(data_width-1 downto 0);

	signal dc_valid_xr_s : std_logic;
	signal dc_valid_xe_s : std_logic;
	signal sample_valid_adap_s : std_logic;
...
--  who_ok           <= who_ok_s;
 ...
 --------------------------------------------------------------------
-- Sensor 1 = referência xr bruto
-- Sensor 2 = erro xe bruto
--------------------------------------------------------------------
xr_raw_s <= signed(accel_z_s(31 downto 16));
xe_raw_s <= signed(accel_z_s(15 downto 0));

--accel_z <= accel_z_s;
who_ok  <= who_ok_s;

--------------------------------------------------------------------
-- Removedor de DC do sensor de referência
--------------------------------------------------------------------
dc_xr_inst : dc_blocker_iir
  generic map (
  ...
  port map (
    ...
    y_out        => xr_dyn_s,
    mean_out     => mean_xr_s,
    valid_out    => dc_valid_xr_s
  );

--------------------------------------------------------------------
-- Removedor de DC do sensor de erro
--------------------------------------------------------------------
dc_xe_inst : dc_blocker_iir
  generic map (
    DATA_WIDTH => data_width,
    EXTRA_BITS => 8,
    SHIFT      => 10
  ...
  port map (
    ...
    y_out        => xe_dyn_s,
    mean_out     => mean_xe_s,
    valid_out    => dc_valid_xe_s
  );

--------------------------------------------------------------------
-- Sinais que entram no adaptativo
--------------------------------------------------------------------
xr <= std_logic_vector(xr_dyn_s);
xe <= std_logic_vector(xe_dyn_s);

accel_z <= xr & xe;

-- Pulso de amostra já alinhado com os sinais sem DC
sample_valid_adap_s <= dc_valid_xr_s and dc_valid_xe_s;

sample_valid_imu <= sample_valid_adap_s;
