...
//    localparam [15:0] PRER_VALUE   = 16'd99;      // ~100 kHz para 50 MHz
    localparam [15:0] PRER_VALUE   = 16'd24;      // ~400 kHz para 50 MHz
    ...
    localparam [7:0] CMD_READ_ACK      = 8'h20; // RD + ACK
    localparam [7:0] CMD_READ_NACK_STO = 8'h68; // RD + NACK + STO
    localparam [7:0] CTR_ENABLE        = 8'h80;
    ...
    // Controle da leitura burst de 2 bytes
    reg        op_read_two;
    reg        read2_phase;
        ...
        if (!rstn) begin
            ...
            case (state)
                ...
                ST_AFTER_CFG_WRITE: begin
                    if (cfg_index < 2'd1) begin
                        cfg_index <= cfg_index + 2'd1;
                        state     <= ST_LOAD_CFG;
                    end else begin
                        cfg_index    <= 2'd2;
                        config_ok    <= !nack_error;
                        nack_error   <= 1'b0;
                        op_read_two  <= 1'b0;
                        op_reg_addr  <= REG_WHO_AM_I;
                        return_state <= ST_AFTER_WHO;
                ...
                ST_AFTER_STATUS: begin
                    ...
                    // STATUS[0] = XLDA -> novo dado do acelerômetro disponível
                    if (!nack_error && rd_byte[0]) begin
                        reading_ok   <= 1'b0;
                ...
                // =====================================================
                // Leitura burst de 2 bytes:
                // OUTZ_L_XL e OUTZ_H_XL em uma única transação
                // =====================================================
                ST_LOAD_DATA_READ: begin
                    nack_error   <= 1'b0;
                    op_read_two  <= 1'b1;
                    op_reg_addr  <= REG_OUTZ_L_XL;
                    return_state <= ST_STORE_DATA_BYTE;
                    state        <= ST_R_TXR1_REQ;
                end

                ST_STORE_DATA_BYTE: begin
                    accel_z    <= {rd_byte1, rd_byte0};
                    reading_ok <= !nack_error;
						  sample_valid <= !nack_error;  // pulso de 1 clock: amostra nova válida
                    state      <= ST_IDLE_WAIT;
                end
                ...
                // =====================================================
                // Generic READ op:
                // 1 byte normal ou 2 bytes em burst
                // =====================================================
                ST_R_TXR1_REQ: begin
                    read2_phase   <= 1'b0;
                    wb_start      <= 1'b1;
                ...
                ST_R_CMD4_REQ: begin
                    ...
                    // Para leitura burst:
                    // primeiro byte: ACK
                    // segundo byte: NACK + STOP
                    if (op_read_two && !read2_phase)
                        wb_start_data <= CMD_READ_ACK;
                    else
                ...
                ST_R_RX_WAIT: begin
                    if (wb_done) begin
                        ...
                        if (op_read_two && !read2_phase) begin
                            // Primeiro byte da leitura burst: OUTZ_L_XL
                            rd_byte0    <= wb_rd_data;
                            read2_phase <= 1'b1;
                            state       <= ST_R_CMD4_REQ;
                        end else begin
                            if (op_read_two) begin
                                // Segundo byte da leitura burst: OUTZ_H_XL
                                rd_byte1 <= wb_rd_data;
                            end else begin
                                // Leitura normal de 1 byte
                                rd_byte <= wb_rd_data;
                            end
